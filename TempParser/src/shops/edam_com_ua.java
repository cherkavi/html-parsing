...
public class edam_com_ua extends TableSinglePage{
	...
	protected Record getRecordFromRow(Node node) {
		// if(((Element)this.parser.getNodeFromNodeAlternative(node, "/td[2]/nobr")).getTextContent().indexOf("На складе")>=0){
			...
			try{
				...
				String priceElementText=priceElement.getTextContent().trim();
				int braceIndex=priceElementText.indexOf('(');
				if(braceIndex>0){
					priceElementText=priceElementText.substring(0,braceIndex);
				}
				priceElementText=priceElementText.toUpperCase().replaceAll("[ $,]", "");
	...
	protected String getSectionNameFromRow(Node node) {
		return this.parser.getTextContentFromNode(node, "/td/font", "");
	}
	...
	protected String getFullHttpUrlToPrice() {
		return "http://edam.com.ua/price.php";
	}

	@Override
	public String getShopUrlStartPage() {
		return "http://edam.com.ua";
	}

	@Override
	protected String getXmlPathToMainBlock() {
		return "/html/body/div/div/div[3]/div[3]/table/tbody/tr[2]/td/table[2]/tbody/tr/td[2]/table/tbody/tr/td/table/tbody/tr/td/table/tbody";
	}
