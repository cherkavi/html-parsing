...
public class technobrend_com_ua extends TableSinglePage{
	...
	protected Record getRecordFromRow(Node node) {
		// if(((Element)this.parser.getNodeFromNodeAlternative(node, "/td[2]/nobr")).getTextContent().indexOf("На складе")>=0){
			...
			try{
				...
				Element nodeName=(Element)this.parser.getNodeFromNode(node, "/td[2]");
	...
	protected String getSectionNameFromRow(Node node) {
		try{
			return this.parser.getTextContentFromNode(node, "/td/h3/strong", "");
		}catch(Exception ex){
			return null;
		}
	...
	protected String getFullHttpUrlToPrice() {
		return "http://tehnobrend.com.ua/price.php";
	}

	@Override
	public String getShopUrlStartPage() {
		return "http://tehnobrend.com.ua";
	}

	@Override
	protected String getXmlPathToMainBlock() {
		return "/html/body/table/tbody/tr[4]/td[2]/table[2]/tbody";
	}
