...
public class technogorod_com_ua extends TableSinglePage{
	...
	protected Record getRecordFromRow(Node node) {
		// if(((Element)this.parser.getNodeFromNodeAlternative(node, "/td[2]/nobr")).getTextContent().indexOf("На складе")>=0){
			...
			try{
				...
				name=nodeName.getTextContent().trim()+((Element)this.parser.getNodeFromNode(node, "/td[2]/a")).getTextContent();
	...
	protected String getFullHttpUrlToPrice() {
		return "http://www.technogorod.com.ua/price.html";
	}

	@Override
	public String getShopUrlStartPage() {
		return "http://www.technogorod.com.ua";
	}
