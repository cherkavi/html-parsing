...
public class mobiles_net_ua extends TableSinglePage{
	...
	protected Record getRecordFromRow(Node node) {
		// if(((Element)this.parser.getNodeFromNodeAlternative(node, "/td[2]/nobr")).getTextContent().indexOf("На складе")>=0){
			...
			try{
				...
				name=((Element)this.parser.getNodeFromNode(node, "/td[1]/a/h1")).getTextContent().trim();
				
				Element priceElement=(Element)this.parser.getNodeFromNode(node, "/td[2]/font");
				...
				int dotIndex=priceElementText.indexOf("(");
				if(dotIndex>0){
					priceElementText=priceElementText.substring(dotIndex);
				}
				priceElementText=priceElement.getTextContent().trim().toUpperCase().replaceAll("[А-ЯA-Z() $,.]", "");
	...
	protected String getFullHttpUrlToPrice() {
		return "http://www.mobiles.net.ua/price.asp";
	}

	@Override
	public String getShopUrlStartPage() {
		return "http://www.mobiles.net.ua";
	}

	@Override
	protected String getXmlPathToMainBlock() {
		return "/html/body/table/tbody/tr/td[2]/table/tbody/tr[2]/td/table[2]/tbody/tr/td/table/tbody/tr/td/table/tbody";
	}
