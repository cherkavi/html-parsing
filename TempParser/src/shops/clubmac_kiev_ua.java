...
public class clubmac_kiev_ua extends TableSinglePage{
	...
	protected Record getRecordFromRow(Node node) {
		// if(((Element)this.parser.getNodeFromNodeAlternative(node, "/td[2]/nobr")).getTextContent().indexOf("На складе")>=0){
			...
			try{
				...
				description=((Element)this.parser.getNodeFromNode(node, "/td[2]")).getTextContent();
				...
				priceUSD=this.getFloatFromString(priceElementText);
