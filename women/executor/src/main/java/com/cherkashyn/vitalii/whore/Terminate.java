...
@Command(scope="spider", name="terminate", description=" stop spider(s) ")
public class Terminate extends ReestrAware {
	...
	protected Object doExecute() throws Exception {
		...
		if(!Spider.Status.WORKING.equals(spiderForExecute.getCurrentStatus())){
			System.out.println("ReturnCode:0");
			System.out.println("spider is NOT running now: "+name);
			return null;
		}
		spiderForExecute.terminate();
		System.out.println("ReturnCode:1");
		System.out.println("spider was terminated: "+name);
		return null;
	}
