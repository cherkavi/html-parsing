...
*/			
	...
	/** получить список всех Action's которые находятся в состоянии IN_PROCESS */
	@SuppressWarnings("unchecked")
	...
	/** для уаказанного номера Action установить состояние */
	...
	/** получить уникальные номера указанных CURRENT_ACTIONS (id_parse_session is null), которые задействованы в процессе по обработке указанного ACTIONS 
	 * @param actionId - уникальный номер Action 
	 * @return список SESSION.ID по данному Action (либо null )
	 */
	...
	/** получить объект сессии из базы данных  
	...
	/** Установить для CurrentAction указанный SessionId 
	...
	/** удалить по указанному Action все данные из таблицы Parse_record по которым сессии находятся в неопределенном, незаконченном состоянии 
	 * <br>
	 * то есть, выражаясь терминами базы данных:
	 * найти в CURRENT_ACTION.ID_ACTIONS такие записи, которые бы имели CURRENT_ACTION.ID_PARSE_SESSION->PARSE_SESSION.ID 
	 * */
	public void removeNotCompleteSessionByAction(Integer id) {
		ConnectWrap connector=StaticConnector.getConnectWrap();
		try{
			ArrayList<Integer> parseSession=new ArrayList<Integer>();
			// ArrayList<Integer> idCurrentAction=new ArrayList<Integer>();
			StringBuffer query=new StringBuffer();
			// получить все записи, которые находятся в процессе, либо же новые 
			query.append("select current_action.id, current_action.id_parse_session, parse_session.id_parse_result from current_action \n");
			query.append("inner join parse_session on parse_session.id=current_action.id_parse_session \n");
			query.append("where current_action.id_actions="+id+" and  (id_parse_result is null or id_parse_result=4 or id_parse_result=3) \n");
			ResultSet rs=connector.getConnection().createStatement().executeQuery(query.toString());
			while(rs.next()){
				// idCurrentAction.add(rs.getInt(1));
				parseSession.add(rs.getInt(2));
			}
			rs.getStatement().close();
			if(parseSession.size()>0){
				String idParseSessionForQuery=this.getIntegerDelimeterComma(parseSession);
				Connection connection=connector.getConnection();
				// удалить данные из таблицы Parse_record
				connection.createStatement().executeUpdate("delete from parse_record where id_session in ("+idParseSessionForQuery+")");
				// удалить данные из таблицы Parse_session
				connection.createStatement().executeUpdate("delete from parse_session where id in ("+idParseSessionForQuery+")");
				// очистить данные в таблице Current_action
				connection.createStatement().executeUpdate("update current_action set id_parse_session=null where id_parse_session in ("+idParseSessionForQuery+")");
				connection.commit();
			}
		}catch(Exception ex){
			System.err.println("DatabaseProxy#removeNotCompleteSessionByAction Exception:"+ex.getMessage());
			return ;
		}finally{
			connector.close();
		}
	...
	/** получить список чисел через запятую */
	private String getIntegerDelimeterComma(List<Integer> list){
		StringBuffer returnValue=new StringBuffer();
		for(int counter=0;counter<list.size();counter++){
			returnValue.append(list.get(counter));
