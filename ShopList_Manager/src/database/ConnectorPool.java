...
public class ConnectorPool implements IConnector{
	 ...
	 * @param poolCount - кол-во элементов в Pool
	 * @throws Exception - если произошла какая-либо ошибка во время попытки соединения с базой данных
	 * */
	public ConnectorPool(String driverName, String fullPath, String userName, String password, int poolCount) throws Exception {
		this.driverName=driverName;
		this.fullPath=fullPath;
		this.user=userName;
		this.password=password;
		if(initPool(poolCount)==false){
			throw new Exception("init Error");
		}
	}
	
	private boolean initPool(int poolCount) throws ClassNotFoundException {
		Class.forName(driverName);
		String connectionString = fullPath;
		GenericObjectPool connectionPool = new GenericObjectPool(null,poolCount);
